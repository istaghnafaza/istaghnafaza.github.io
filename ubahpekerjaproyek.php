...
// ");
$pekerja = query("SELECT p.id_pekerja, u.tgl_mulai, u.tgl_selesai, q.nama_proyek, u.id, p.nama_pekerja AS 'Nama Pekerja', b.nama_kategori AS 'Kategori Pekerja', u.upah AS 'Upah Harian'
                FROM tbl_pivot_pekerjaproyek u 
                JOIN tb_pekerja p ON u.id_pekerja = p.id_pekerja 
                JOIN tb_proyek q ON q.id_proyek = u.id_proyek
...
// cek apakah tombol submit sudah ditekan atau belum
    if ( isset($_POST["submit"]) ) {
        ...
        if(ubahdataPekerja($_POST) != 0){
            echo "
                <script>
                alert('Data berhasil di tambahkan');
                </script>
            ";
            header('Location: tambahpekerja.php?id=' . urlencode($idproyek));
            exit();
        } else {
            echo "<script>
   ...
   <?php include 'navbar.php'; ?>
    ...
    <section class="py-5">
      <div class="container">
        ...
        <h3 class="text-center mb-5" >Project <?= $baris["nama_proyek"]?></h3>
        <div class="row"> 
                ...
                <div class="row">
                    <div class="col-md-4">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="upah" name="upah" value="<?= $baris["Upah Harian"]?>">
                        </div>
                        <div class="mb-3">
                            <label for="tgl_mulai" class="form-label">Tanggal Mulai</label>
                            <input type="date" class="form-control" id="tgl_mulai" name="tgl_mulai" value="<?= $baris["tgl_mulai"]?>">
                        </div>
                        <div class="mb-3">
                            <label for="tgl_selesai" class="form-label">Tanggal Selesai</label>
                            <input type="date" class="form-control" id="tgl_selesai" name="tgl_selesai" value="<?= $baris["tgl_selesai"]?>">
                        </div><?php  }?>
